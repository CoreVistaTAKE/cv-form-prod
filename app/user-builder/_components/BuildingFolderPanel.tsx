...
type CreateRes = {
  ok?: boolean; traceId?: string; token?: string; statusPath?: string; user?: string; seq?: string; bldgFolderName?: string;
};
type Props = { createUrl?: string; statusUrl?: string; defaultUser?: string | null; defaultHost?: string | null; };
...
function required(url: string, name: string){ if(!url){ throw new Error(`${name} が未設定です。.env.local に ${name} を定義してください。`); } }
...
export default function BuildingFolderPanel({ createUrl, statusUrl, defaultUser, defaultHost }: Props){
  ...
  const onRun = useCallback(async ()=>{
    setError(undefined); setTraceId(undefined); setToken(undefined); setStatusPath(undefined);
    try{
      ...
      if(!res.ok){ const text=await res.text().catch(()=> ""); throw new Error(`CreateFormFolder が失敗: ${res.status} ${text}`); }
      ...
      if(!tkn || !stPath) throw new Error("Flow の応答に token または statusPath が含まれていません。");

      setTraceId(json.traceId); setToken(tkn); setStatusPath(stPath);
      try{ localStorage.setItem("cv:lastBuild", JSON.stringify({ user, bldg: bldg.trim(), statusPath: stPath })); }catch{}
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-3">
        <button className="btn" onClick={onRun} disabled={!canRun || running} title={!canRun ? "建物名を入力してください" : "建物フォルダを作成します"}>

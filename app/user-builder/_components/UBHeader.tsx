 ...
 * 対象外(非適用)設定パネル
 *
 * - セクション（ページ）ごとに「一括で非表示候補」にするトグル
 * - フィールドごとに「個別に非表示候補」にするチェック
 *
 * ※ 現時点ではフォーム本体の schema には反映せず、
 *    「どれを対象外にしたいか」を UI 上で確認・メモする用途。
 */
export default function ExistingExcludePanel() {
  ...
  const togglePageExclude = (pageId: string, fieldsOfPage: Field[]) => {
    ...
    // ページ一括トグル時は、そのページ配下のフィールドもまとめて ON/OFF
    setExcludedFieldIds((prev) => {
      ...
      const nowPageExcluded = excludedPages.has(pageId);
      // nowPageExcluded = true → 今回は OFF にする
      // nowPageExcluded = false → 今回は ON にする
      if (nowPageExcluded) {
        // 全解除
        for (const id of ids) {
          next.delete(id);
  ...
  const toggleFieldExclude = (pageId: string, field: Field) => {

...
export default function UserBuilderPanels({
  ...
  // ページ初期表示時に、あれば cv:lastBuild を復元
  useEffect(() => {
    try {
      const raw = localStorage.getItem('cv:lastBuild');
      if (raw) {
        const obj = JSON.parse(raw);
        if (obj && obj.statusPath) {
          setStatusInfo(obj);
        }
      }
    } catch {
  ...
  // BaseSystem にリセット（建物を選ばず、標準 form_base.json を読み込み、ステータスは空にする）
  ...
  return (
    <div className="space-y-6">
      ...
      <SectionCard id="intake" title="建物用フォームを読み込む（サーバ上の JSON）">
        <div className="flex items-center gap-2" style={{ flexWrap: 'wrap' }}>
          ...
          <button
            ...
            disabled={!picked || intakeBusy}
          >
            読込
          </button>
          <button
            className="btn-secondary"
            onClick={reloadBuildings}
            disabled={intakeBusy}
          >
            再取得
          </button>
        ...
        <div className="space-y-3">
          {sectionPages.map((p) => {
            ...
            const allFieldExcluded =
              fids.length > 0 && fids.every((id) => excludedFields.has(id));
      ...
      <SectionCard id="status" title="ステータス">
        {statusInfo && statusInfo.statusPath ? (
          <BuildStatus
          ...
          <div className="text-xs text-slate-500">
            建物用フォームを読み込むと、ここにその建物の URL / QR コードが表示されます。
          </div>
        )}
      </SectionCard>
    </div>

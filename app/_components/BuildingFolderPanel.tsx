    ...
    <section style={{
      ...
      <div style={{ display: 'flex', gap: 12, alignItems: 'center', flexWrap: 'wrap' }}>
        ...
        <button
          ...
          style={{
            padding: '10px 16px',
            background: stage === 'running' ? 'var(--muted)' : 'var(--brand)',
            color: '#FFFFFF',
            border: 'none',
            borderRadius: 6,
        ...
        {stage !== 'idle' && (
          <button onClick={reset} style={{ padding: '10px 12px', border: '1px solid var(--border)', borderRadius: 6, background: '#FFF' }}>
            クリア
          </button>
        )}
      </div>
      ...
      <div style={{ marginTop: 16 }}>
        <div style={{ height: 8, background: 'var(--muted)', borderRadius: 9999, overflow: 'hidden' }}>
          <div style={{ width: `${progress}%`, height: '100%', background: 'var(--brand)', transition: 'width 0.8s ease' }} />
        </div>
          ...
          {marks.map((m) => (
            <div key={m.key} style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
              <span style={{
                ...
                background: progress >= m.pct ? 'var(--brand)' : 'var(--border)'
              }} />
      ...
      {/* 結果/エラー表示 */}
      {result && (
        <div style={{ marginTop: 16, padding: 12, border: '1px solid var(--border)', borderRadius: 6, background: 'var(--muted)' }}>

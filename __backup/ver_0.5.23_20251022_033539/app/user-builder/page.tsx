...
export default function UserBuilderPage(){
  ...
  // 既存の「読み込む」動作（ファイル選択→ローカル保存→hydrate）
  ...
  return (
    <div className="space-y-6">
      {/* ページ内ヘッダー（スマホ小さめ／右ハンバーガー／4リンク） */}
      <header data-ub-header className="ub-hdr" style={{ borderBottom:"1px solid var(--border)", background:"var(--panel)", color:"var(--text)", padding:"8px 12px", marginBottom:12 }}>
        <div className="ub-bar" style={{ display:"flex", alignItems:"center", justifyContent:"space-between", gap:8 }}>
          <h1 className="form-title" style={{ margin:0, fontSize:15, lineHeight:"20px" }}>ユーザー用ビルダー</h1>
          <details className="ub-menu" style={{ position:"relative" }}>
            <summary aria-label="menu" style={{ listStyle:"none", cursor:"pointer", fontSize:16, lineHeight:"16px", border:"1px solid var(--border)", borderRadius:6, padding:"6px 10px" }}>☰</summary>
            <nav className="ub-nav" style={{ position:"absolute", right:0, top:"100%", background:"var(--panel)", border:"1px solid var(--border)", borderRadius:8, marginTop:6, padding:8 }}>
      ...
      {/* 1) ユーザー用ビルダー（…）【ユーザー用ベースを読み込む】（既存のもの／URLと保存名の枠） */}
      <section id="userbase" className="card">
        <div className="form-title mb-2">ユーザー用ビルダー（フォーム設定・対象外・テーマ）【ユーザー用ベースを読み込む】</div>
        ...
        <div className="text-right mt-2">
          <button className="btn" onClick={handleLoadUserBase}>ユーザー用ベースを読み込む</button>
        </div>
      </section>

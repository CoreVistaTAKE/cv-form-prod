...
<header data-ub-header className="ub-hdr" style={{borderBottom:"1px solid var(--border,#E5E7EB)",background:"var(--background,#FFFFFF)",color:"var(--foreground,#111827)",padding:"8px 12px",marginBottom:12}}>
  <div className="ub-bar" style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:8}}>
    <h1 style={{margin:0,fontSize:15,lineHeight:"20px"}}>対象外(非適用)設定</h1>
    <details className="ub-menu" style={{position:"relative"}}>
      <summary aria-label="menu" style={{listStyle:"none",cursor:"pointer",fontSize:16,lineHeight:"16px",border:"1px solid var(--border,#E5E7EB)",borderRadius:6,padding:"6px 10px"}}>☰</summary>
      <nav className="ub-nav" style={{position:"absolute",right:0,top:"100%",background:"var(--background,#FFFFFF)",border:"1px solid var(--border,#E5E7EB)",borderRadius:8,marginTop:6,padding:8}}>
        <a href="#userbase" style={{display:"block",margin:"6px 0",fontSize:12}}>ユーザー用ビルダー（読み込み）</a>
        <a href="#color"    style={{display:"block",margin:"6px 0",fontSize:12}}><a id="color"></a>フォームカラー設定（フォームに反映）</a>
        <a href="#exclude"  style={{display:"block",margin:"6px 0",fontSize:12}}><a id="exclude"></a>対象外(非適用)設定</a>
        <a href="#folder"   style={{display:"block",margin:"6px 0",fontSize:12}}><a id="folder"></a>
<BuildingFolderPanel />
建物フォルダ作成とURL発行</a>
      </nav>
    </details>
  ...
  <style>{`
    .ub-menu .ub-nav { display:none; }
    .ub-menu[open] .ub-nav { display:block; }
    @media (max-width: 480px) {
      .ub-hdr h1 { font-size:14px; line-height:18px; }
      .ub-nav a { font-size:12px; }
    }
  `}</style>
</header>
<section id="userbase" data-ub-userbase style={{border:"1px solid var(--border,#E5E7EB)",borderRadius:8,padding:16,marginBottom:16,background:"var(--background,#FFFFFF)",color:"var(--foreground,#111827)"}}>
  <h2 style={{margin:0,marginBottom:8,fontSize:14}}>対象外(非適用)設定</h2>
  <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(240px,1fr))",gap:12}}>
    <label style={{fontSize:12,display:"flex",flexDirection:"column"}}>
      URL
      <input placeholder="https://example.com/user-base.json" style={{padding:8,border:"1px solid var(--border,#D1D5DB)",borderRadius:6,background:"var(--background,#FFFFFF)",color:"var(--foreground,#111827)"}}/>
    </label>
    <label style={{fontSize:12,display:"flex",flexDirection:"column"}}>
      保存名
      <input placeholder="user_base_01" style={{padding:8,border:"1px solid var(--border,#D1D5DB)",borderRadius:6,background:"var(--background,#FFFFFF)",color:"var(--foreground,#111827)"}}/>
    </label>
  </div>
  <div style={{marginTop:12}}>
    <button style={{padding:"8px 12px",borderRadius:6,border:"1px solid var(--border,#E5E7EB)",background:"var(--primary,#111827)",color:"#FFFFFF",cursor:"pointer",minWidth:160}}>ユーザー用ベースを読み込む</button>
  </div>
</section>

...
const RESERVED = new Set([
  "点検日","ReportSheet（タブ名）","会社名","建物名","点検者名","【2名以上の点検】共同報告グループID"
]);
...
export function Wizard(){
  ...
  const sectionIdxs=useMemo(()=>pages.map((p,i)=>p.type==="section"?i:-1).filter(i=>i>=0),[pages]);
  const lastSectionIndex=useMemo(()=>sectionIdxs.length?sectionIdxs[sectionIdxs.length-1]:-1,[sectionIdxs]);
  ...
  const TopNav = ({left,right}:{left:()=>void;right:()=>void;})=>(
    <div className="flex items-center justify-between mb-2">
      <button className="btn-blue-light btn-nav" onClick={left}>← 戻る</button>
      <button className="btn-blue btn-nav" onClick={right}>次へ →</button>
    </div>
  );

  function loadForEdit(id:string){
    const item = responses.getById(id); if(!item) return;
    setEditingId(id);
    ...
    if(fields.some(f=>f.label==="【2名以上の点検】共同報告グループID")){
      setValue("【2名以上の点検】共同報告グループID", (item.groupId||"").toString().replace(/[^\d]/g,""));
    }
  ...
  function saveCurrent(submit:boolean){
    ...
    if(editingId){ responses.update(editingId, { building, company, inspector, groupId, dateISO, sheet, values }); }
    else{ responses.create({ building, company, inspector, groupId, dateISO, sheet, values }); }

    if(submit && completeIndex>=0){
      setWorking(true); setIdx(completeIndex);
      setTimeout(()=>{
  ...
  const stepIndices = useMemo(()=>{
    ...
    return arr;
  },[infoIndex,reviseIndex,reviseListIndex,basicIndex,prevIndex,sectionIdxs,reviewIndex,completeIndex]);
  ...
  const headerExcerpt = (
    <div className="card" style={{marginBottom:12}}>
      ...
      <div className="form-text" style={{opacity:.9}}>
        {(meta.fixedBuilding||"") && <>建物：{meta.fixedBuilding}　</>}
        {(meta.fixedCompany||"") && <>会社：{meta.fixedCompany}</>}
      </div>
      {Array.isArray(meta.descriptions)&&meta.descriptions[0] && <div className="form-text" style={{opacity:.8, marginTop:4}}>{meta.descriptions[0]}</div>}
      <div className="progress mt-2"><div className="bar" style={{width:`${percent}%`}}/></div>
  ...
  return (
    <div className="card">
      ...
      {isInfo&&(
        <div>
          <div className="card">
            ...
            {Array.isArray(meta.rules)&&meta.rules.length>0&&(<ul>{meta.rules.map((r,i)=>(<li key={i} className="form-text">{r}</li>))}</ul>)}
          </div>
          <div className="flex items-center justify-between mt-3">
            <button className="btn-yellow-light btn-nav" onClick={()=>{ if(reviseIndex>=0) setIdx(reviseIndex); }}>← 修正ページ</button>
            <button className="btn-blue btn-nav" onClick={()=>{ if(basicIndex>=0) setIdx(basicIndex); }}>基本情報へ →</button>
          </div>
        </div>
      )}
        ...
        <div>
          <TopNav
            ...
            right={()=>{
              ...
              if(!ok) return;
              const fs = sectionIdxs[0]; if(fs!==undefined) setIdx(fs);
            }}
          />
          <div className="mb-2"><div className="form-title">{currentPage.title||"前回点検時の状況"}</div></div>
          <div className="card"><PrevGroupsView pages={pages} fields={fields} meta={meta} responses={responses} watchBld={bWatch} watchSheet={sWatch} isFromRevise={isFromRevise} revisePrevItem={revisePrevItem}/></div>
          <TopNav
            left={()=>setIdx(basicIndex>=0?basicIndex:0)}
            right={()=>{
              const ok = validatePageRequired(basicIndex);
        ...
        <div>
          <TopNav
            ...
            right={()=>{
              ...
              if(!ok) return;
              if(idx===lastSectionIndex){ setIdx(reviewIndex); } else { setIdx(idx+1); }
            }}
          />
          <div className="mb-2"><div className="form-title">{currentPage.title||"セクション"}</div>{currentPage.description&&<div className="form-text" style={{opacity:.9}}>{currentPage.description}</div>}</div>
          <div className="card">
            <div className="space-y-3">
              {fields.filter(f=>f.pageId===currentPage.id).map(f=>(
                <div key={f.id} className="card">
                  <label className="form-text">{f.label}{f.required&&<span style={{color:"#f99", marginLeft:8}}>＊</span>}</label>
                  {f.type==="text"&&(<input className="input mt-2" {...register(f.label)} />)}
                  {f.type==="textarea"&&(<textarea className="input mt-2" style={{height:110}} {...register(f.label)} />)}
                  ...
                  {f.type==="select"&&(
                    <select className="input mt-2" {...register(f.label)}>
                      ...
                      {(f.options||[]).slice(0,10).map(opt=>(<option key={opt} value={opt}>{opt}</option>))}
                    </select>
                  )}
                </div>
              ))}
      ...
      {isReview&&(
        <div>
          <div className="mb-2"><div className="form-title">最終確認</div><div className="form-text" style={{opacity:.9}}>入力した箇所をセクションごとに表示します。この入力を元に報告書を作成します。</div></div>
          <div className="space-y-3">
            {pages.filter(p=>p.type==="section").map((sp,i)=>{
              ...
              return (
                <div key={sp.id} className="card">
                  ...
                  <div className="space-y-2">
                    {entries.map(([k,v])=>(
                      <div key={k} className="flex"><div style={{width:220, color:"#cfe0ff"}}>{k}</div><div>{String(v)}</div></div>
                    ))}
                  </div>
                </div>
              );
          ...
          <div className="flex items-center justify-between mt-3">
            <button className="btn-blue-light btn-nav" onClick={()=>setIdx(lastSectionIndex)}>← 戻る</button>
            <button className="btn-yellow btn-nav" onClick={()=>saveCurrent(true)}>報告書作成 →</button>
          </div>
        </div>
      )}
      ...
      {isComplete&&(
        <div className="space-y-3">
          {working ? (
            ...
            <div className="card text-center">
              <p className="form-text" style={{fontSize:16, marginTop:6}}>フォーム入力、おつかれさまでした。よく休憩して、次も安全にいきましょう。</p>
              {pdfUrl ? <a className="btn btn-nav" href={pdfUrl} download="report.pdf">PDFをダウンロード（テスト用）</a> : <span className="form-text" style={{opacity:.8}}>PDFは次工程（Excel連携）で本格生成します。</span>}
            </div>
          )}
          <div className="text-right"><button className="btn-secondary" onClick={()=>{ setEditingId(undefined); setIsFromRevise(false); setIdx(0); }}>最初に戻る</button></div>
        </div>
      )}
    </div>
  );
...
function ReviseBlock({setIdx,indices,loadForEdit}:{setIdx:any;indices:any;loadForEdit:(id:string)=>void;}){
  ...
  useEffect(()=>{ if(meta.fixedBuilding){ setReviseBuilding(meta.fixedBuilding); } },[meta.fixedBuilding]);
  const buildings = useMemo(()=>{ const set = new Set(responses.list.map(r=>r.building).filter(Boolean)); return Array.from(set); },[responses.list]);
  const respListFiltered = useMemo(()=>{
    const bld = reviseBuilding || meta.fixedBuilding || ""; if(!bld) return [];
  ...
  return (
    <>
      {!meta.fixedBuilding && (<div className="card">
        <div className="form-title mb-1">建物を選択</div>
        <select className="input" value={reviseBuilding} onChange={e=>{setReviseBuilding(e.target.value); setReviseRespId("");}}>
          <option value="">選択してください</option>
          {Array.from(new Set(buildings)).map(b=>(<option key={b} value={b}>{b}</option>))}
        </select>
      </div>)}
      {meta.fixedBuilding && (<div className="card"><div className="form-text">建物：{meta.fixedBuilding}（固定）</div></div>)}

      <div className="card">
        <div className="form-title mb-1">点検者名で絞り込み（任意）</div>
        <input className="input" placeholder="例：中尾" value={reviseInspector} onChange={e=>setReviseInspector(e.target.value)}/>
      </div>

      <div className="card">
        <div className="form-title mb-1">点検日（YYYYMMDD）を選択</div>
        <select className="input" value={reviseRespId} onChange={e=>setReviseRespId(e.target.value)} disabled={!(reviseBuilding||meta.fixedBuilding)}>
          <option value="">選択してください</option>
          {respListFiltered.map(r=>(<option key={r.id} value={r.id}>{r.sheet}（{r.inspector}）</option>))}
        </select>
      </div>

      {!!reviseRespId && (()=>{ const item = responses.getById(reviseRespId)!; const entries = Object.entries(item.values||{}).filter(([k,v])=>String(v||"").trim()!==""); return (
        <div className="card">
          <div className="form-title mb-1">選択した回答の一覧（{item.sheet} / {meta.fixedBuilding||item.building} / {item.inspector||""}）</div>
          <div className="space-y-2">
            {entries.length===0 && <div className="form-text" style={{opacity:.7}}>入力済み項目はありません。</div>}
            {entries.length>0 && entries.map(([k,v],i)=>(<div key={i} className="flex"><div style={{width:220, color:"#cfe0ff"}}>{k}</div><div>{String(v)}</div></div>))}
          </div>
        </div>
      ); })()}

      <div className="flex items-center justify-between mt-3">
        <button className="btn-red-light btn-nav" onClick={()=>setIdx(indices.infoIndex>=0?indices.infoIndex:0)}>← 戻る</button>
        <button className="btn-yellow btn-nav" disabled={!reviseRespId} onClick={()=>loadForEdit(reviseRespId!)}>{indices.reviseListIndex>=0?"修正する回答ページへ →":"回答の修正へ →"}</button>
      </div>
    </>
  );
}
...
function ReviseListBlock({setIdx,indices}:{setIdx:any;indices:any;}){
  return (
    <div className="space-y-3">
      <div className="card"><div className="form-text">修正対象が選ばれていません。</div></div>
      <div className="flex items-center justify-between mt-3">
        <button className="btn-blue-light btn-nav" onClick={()=>setIdx(indices.reviseIndex>=0?indices.reviseIndex:0)}>← 戻る</button>
        <button className="btn-blue btn-nav" onClick={()=> setIdx(indices.basicIndex>=0?indices.basicIndex:(indices.reviseIndex+1))}>基本情報へ →</button>
      </div>
    </div>
  );
}
...
function BasicBlock({setIdx,indices,register,pages,fields,validateCurrent}:any){
  ...
  return (
    <div>
        ...
        <div className="space-y-3 mt-3">
          {fs.map((f:any)=>(
            <div key={f.id} className="card">
              ...
              {f.type==="text"&&(<input className="input mt-2" {...register(f.label)} placeholder={f.placeholder||""} />)}
              {f.type==="date"&&(<input type="date" className="input mt-2" {...register(f.label)} />)}
              {f.type==="number"&&(<input type="text" className="input mt-2" {...register(f.label)} />)}
            </div>
          ))}
      ...
      <div className="flex items-center justify-between mt-3">
        <button className="btn-red-light btn-nav" onClick={()=>setIdx(indices.infoIndex>=0?indices.infoIndex:0)}>← 戻る</button>
        <button className="btn-blue btn-nav" onClick={()=>{
...
function PrevGroupsView({pages,fields,meta,responses,watchBld,watchSheet,isFromRevise,revisePrevItem}:any){
  const bld = (meta.fixedBuilding || watchBld || "").trim();
  const ymd = (watchSheet || "").trim();
  const item = (isFromRevise && revisePrevItem) ? revisePrevItem : (bld ? responses.latestBefore(bld, ymd) : undefined);
  if(!item) return <div className="form-text" style={{opacity:.7}}>前回の入力済み項目はありません。</div>;
  ...
  for(const sp of pages.filter((p:any)=>p.type==="section")){
    ...
    const items:any[] = [];
    for(const f of fs){
      const raw = item.values?.[f.label];
      const s = (raw===undefined||raw===null) ? "" : String(raw).trim();
      if(s!==""){ items.push({label:f.label, value:s}); seen.add(f.label); }
    }
    if(items.length>0) groups.push({ title: sp.title || "セクション", items });
  }
  const others:any[] = [];
  for(const [k,v] of Object.entries(item.values||{})){
    if(RESERVED.has(k)) continue;
    const s = String(v||"").trim(); if(!s) continue;
    if(!seen.has(k)) others.push({label:k, value:s});
  }
  if(others.length>0) groups.push({title:"その他", items:others});
  return (
    <div className="space-y-3">
      {groups.map((g,i)=>(
